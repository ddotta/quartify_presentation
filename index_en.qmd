...
- 📐 **Multiple features**: callout, tabset, mermaid...
...
- 📑 **[Tabsets](https://quarto.org/docs/interactive/layout.html#tabset-panel)**: Organization in tabs
...
### From Cerise and tar.gz file

```r
install.packages("~/CERISE/01-Espace-de-Partage/SSP/BMIS/__packages__/quartify_1.1.1.tar.gz", repos = NULL, type = "source")
```
...
rtoqmd(
  ...
  theme = NULL,           # Quarto theme
  render_html = TRUE,     # Generate HTML
  output_html_file = NULL,# Output .html file (auto if NULL)
  open_html = FALSE,      # Open HTML automatically
)
```

[Complete Syntax](https://ddotta.github.io/quartify/reference/rtoqmd.html)

---

## 🔧 Customization parameters for rtoqmd()


::: {.incremental}
- **theme**: Among 25 Quarto themes (default: "cosmo")
- **code_fold**: Fold code blocks (default: FALSE)
- **number_sections**: Number sections? (default: TRUE)
...
::: {.incremental}
- **render_html**: Generate HTML? (default: TRUE)
- **output_html_file**: Output HTML filename
- **open_html**: Open HTML? (default: FALSE)
...
rtoqmd(
  ...
  output_file = "analysis_quality.qmd",
...
rtoqmd_dir(
  ...
  author = "Your name",   # Author
  render_html = FALSE,    # Generate html file
  output_html_dir = NULL, # Path to the html output directory
  exclude_pattern = NULL  # Pattern to exclude files
)
```

[Complete Syntax](https://ddotta.github.io/quartify/reference/rtoqmd_dir.html)
...
rtoqmd_dir(
  ...
  book_title  = "Documentation_SSP_project_2026",
  author = "My team",
  theme = "cosmo",
  render_html = TRUE
)
...
You can include private notes or development comments in your R scripts that won't appear in the rendered output. Comments that start with `#` immediately followed by a non-space character are completely ignored: vos scripts R qui n'apparaîtront pas dans le rendu final. Les commentaires qui commencent par `#` immédiatement suivi d'un caractère non-espace sont complètement ignorés :

```r
# This comment WILL appear in the output

#This will NOT appear (no space after #)
...
:::
::: {.column width="50%"}
Generates a formatted callout in the HTML file:  

![](images/fonctions.png)
:::
:::
...
::: {.incremental}
- ⚠️ The `apply_styler` parameter allows you to **permanently** format your R script files.
...
## 🔄 GitLab CI (internal MAASA) - Configuration

### File .gitlab-ci.yml

See [this project](https://gitlab.forge.agriculture.rie.gouv.fr/ssp/bmis/divers/quartify_gur).
...
generate-docs:
  ...
  stage: build
  script:
    - R -e "quartify::rtoqmd_dir('scripts/', render_html = TRUE)"
  artifacts:
    paths:
      - scripts/**/*.qmd
      - scripts/**/*.html
...
::: {.incremental}
- 📦 **GitHub**: [github.com/ddotta/quartify](https://github.com/ddotta/quartify)
- 📖 **Documentation site**: [ddotta.github.io/quartify](https://ddotta.github.io/quartify/)
- 🌐 **Web version**: [quartify.lab.sspcloud.fr](https://quartify.lab.sspcloud.fr/)
:::
